...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"[corr={correlation_id}]   - DEV_MOCK_RAPIDAPI: {settings.DEV_MOCK_RAPIDAPI}")
    logger.info(f"[corr={correlation_id}]   - DEV_MOCK_EPHEMERIS: {settings.DEV_MOCK_EPHEMERIS}")

    if settings.DEV_MOCK_NATAL or settings.DEV_MOCK_RAPIDAPI or settings.DEV_MOCK_EPHEMERIS:
        logger.warning(f"[corr={correlation_id}] ⚠️  ATTENTION: Mode MOCK activé! Les calculs astrologiques ne sont pas réels.")
    else:
        logger.info(f"[corr={correlation_id}] ✅ Mode PRODUCTION: Utilisation des APIs réelles RapidAPI")
