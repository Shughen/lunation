...
## 2️⃣ Flow: calendar/month - getMonth

### 📍 Navigation
- [ ] Aller sur `/calendar/month`
- [ ] Vérifier que l'écran s'affiche avec :
  - [ ] Header avec mois/année actuel
  - [ ] Boutons navigation ← →
...
- [ ] Vérifier que le loader s'affiche brièvement
- [ ] Vérifier que les données s'affichent :
  - [ ] Summary cards (Nouvelles Lunes, Pleines Lunes, Éclipses, Événements)
  - [ ] Section "🌙 Événements du Mois"
  - [ ] Section "🏰 Mansions Lunaires"
- [ ] **Console logs à vérifier :**
  - Pas d'erreur `[CalendarMonth] Date invalide`
  - Pas d'erreur `[CalendarMonth] Erreur chargement`
- [ ] **Network à vérifier :**
  - Requête `GET /api/calendar/month?year=YYYY&month=MM` (mois actuel)
  - Status `200`
  - Response contient `summary` et `days` (ou structure attendue)

**Étape 2 : Navigation mois précédent/suivant**
- [ ] Cliquer sur ← (mois précédent)
- [ ] Vérifier que le mois change dans le header
- [ ] Vérifier que les données se rechargent
- [ ] Cliquer sur → (mois suivant)
- [ ] Vérifier que le mois change
- [ ] Vérifier que les données se rechargent
- [ ] **Console logs à vérifier :**
  - Pas d'erreur `[CalendarMonth] Impossible de changer de mois`
  - Pas d'erreur `[CalendarMonth] Nouvelle date invalide`
- [ ] **Network à vérifier :**
  - Nouvelle requête `GET /api/calendar/month?year=YYYY&month=MM` avec le nouveau mois
  - Status `200`
...
- [ ] Vérifier que l'erreur est affichée :
  - [ ] Message d'erreur visible (pas juste "Error")
  - [ ] Bouton "Réessayer" présent
- [ ] Cliquer sur "Réessayer"
- [ ] Vérifier que le loader s'affiche
- [ ] **Console logs à capturer :**
  - `[CalendarMonth] Erreur chargement: ...`
  - Message d'erreur complet
- [ ] **Network à capturer :**
  - Status code de l'erreur (500, 502, timeout, etc.)

**Scénario 2 : Erreur 404 ou 500**
- [ ] Simuler une erreur backend (modifier temporairement l'endpoint)
- [ ] Aller sur `/calendar/month`
- [ ] **Vérifier que l'erreur est lisible :**
  - [ ] Message d'erreur affiché (pas de crash)
  - [ ] Bouton "Réessayer" fonctionne
- [ ] **Console logs à capturer :**
  - `[CalendarMonth] Erreur chargement:` avec détails
  - `err.response?.status` et `err.response?.data`

**Scénario 3 : Date invalide (edge case)**
- [ ] Si possible, forcer une date invalide (modifier temporairement le code)
- [ ] Vérifier que l'app gère l'erreur :
  - [ ] Message "Erreur: date invalide" affiché
  - [ ] Bouton "Réinitialiser" présent
- [ ] **Console logs à capturer :**
  - `[CalendarMonth] Date invalide, réinitialisation`
  - `[CalendarMonth] Impossible de charger: date invalide`

### 📸 Captures à faire si échec

1. **Screenshot de l'écran** avec l'erreur
2. **Console logs complets** :
   - Tous les `[CalendarMonth]` logs
   - Erreurs `console.error`
3. **Network request/response** :
   - URL complète avec query params
   - Response status + body
4. **État de l'app** :
...
## 3️⃣ Flow: transits/overview - getOverview
...
### Flow 2: calendar/month
- [ ] ✅ Success path (chargement + navigation)
- [ ] ❌ Error path testé (backend down, 404/500, date invalide)
- [ ] 📸 Captures faites si échec

### Flow 3: transits/overview
